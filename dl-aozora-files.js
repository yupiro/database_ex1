...
var URL_RNKING = "http://www.aozora.gr.jp/access_ranking/2014_xhtml.html";
var MAX_RANK = 30;
...
// ランキングページをダウンロード
client.fetch(URL_RNKING, function (err, $, res) {
    if (err) { console.log("DL err"); return; }
    // ランキングのテーブルを全行取得
    ...
    for( var i = 0; i < tr.length; i++) {
        ...
        var name = link.text().replace(/(^\s+|\s+S)/,"");
        // console.log(rank, name, href);
        if(isNaN(rank) || rank > MAX_RANK) continue;
        // 相対パスを絶対パスに変換
        href = URL.resolve(URL_RNKING, href);
